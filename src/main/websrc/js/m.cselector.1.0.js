...
更新日志：
版本1.0.16 日期20161230 增加zjs.tdfixed函数 参数为jq对象table td固定宽度模式thead加样式tfixed 属性basewidth表示基准宽度 td里加width宽度 所有td的宽总和为基准宽度  td可以加bwidth属性 表示多余宽度分配比例 0－1 所有总和为1
...
zjs.tdfixed = function ($id) {
    if ($id.children("thead.tfixed").length > 0) {//如果是表格模式 并启动固定宽模式
        if ($id.children("thead.tfixed").attr("basewidth")) {//如果有基础宽度 那么就拿浏览器宽－基础宽
            ...
            var fpwidth = tablewidth - basewidth - (tdlength * 10);//剩余可分配宽度
            ...
            $id.children("thead.tfixed").children("tr.trth").find("td").each(function (tdi, tdv) {
                ...
                if ($td.attr("width")) {//如果这一列固定宽度 就把所有内容列都加上固定宽度
                    ...
                    $tbody.each(function (tri, trv) {//循环每一行的这一列
                        var $tr = $(trv);
                        if ($tr.find("td").length == tdlength) {//如果td数量和表头一样 再设置
                            var $std = $tr.find("td").eq(tdi);
                            if (!$std.attr("tfixed")) {
                                $std.attr("tfixed", "1");
                                $std.html("<div class='tdfixed' style='width:" + tdwidth + "px;' title='" + $std.text() + "'>" + $std.html() + "</div>");
                            }
                        }
                    });
                }
                    ...
                    if ($id.attr("lockall")) {
                        ...
                        if ($id.attr("scrollToRefresh")) {
                            if ($id.hasClass("ctable")) {
                                var tdlength = $id.find("thead tr").first().children("td").length || 100;
                                $body.append("<tr><td colspan='" + tdlength + "'><div class='pagerpostion'><a name='pager" + that.uipageindex + "'>" + that.uipageindex + "</a></div></td></tr>");
                            }
                            else
